...
class UtilSwagger(IHandlerEndpoint):
    ...
    def get_swagger_description_path(self, a_item, a_with_id):
        ...
        w_path = "/$endpoint_crud/" + a_item["plural"]
        if a_with_id:
            w_path = w_path + "/{id}"
        return w_path
        ...
        if a_service.has_post():
            ...
            if "post" in w_extra_path.keys():
                for w_path in w_extra_path["post"]:
                    ...
                    if (
                        not "/$service/" + a_service.get_name() + "/" + w_path
                        in a_path.keys()
                    ):
    ...
    def get_swagger_description(self, a_item, a_path):
        ...
        if a_item["multipart"]:
            ...
            a_path[
                self.get_swagger_description_path(a_item, False) + "/blob/base64"
            ] = {
                "post": {
